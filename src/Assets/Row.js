...
function Row({gridNums, setGridNums}) {
    ...
    const changeTitle = (x) => {
        if (x !== gridNums.a) {
          ...
          let compSum = (slideLock.b ? 0 : gridNums.b) + ( slideLock.c ? 0 : gridNums.c )  + (slideLock.d ? 0 : gridNums.d) + (slideLock.g ? 0 : gridNums.g);
          //console.log(diff)
          ...
          let ndiff = (slideLock.g ? gridNums.g : gridNums.g - (gridNums.g / compSum) * diff);
          //console.log(x,gdiff,xdiff,sdiff,(x+gdiff+xdiff+sdiff))
          setGridNums({ ...gridNums, a: x, b: gdiff, c: xdiff, d: sdiff, g: ndiff });
        }
    };
    ...
    const changeGraph = (x) => {
        if (x !== gridNums.b) {
            ...
            let compSum = ( slideLock.a ? 0 : gridNums.a ) + ( slideLock.c ? 0 : gridNums.c )  + (slideLock.d ? 0 : gridNums.d) + (slideLock.g ? 0 : gridNums.g);
            //console.log(diff)
            ...
            let ndiff = (slideLock.g ? gridNums.g : gridNums.g - (gridNums.g / compSum) * diff);
            //console.log(x,gdiff,xdiff,sdiff,(x+gdiff+xdiff+sdiff))
            setGridNums({ ...gridNums, a: tdiff, b: x, c: xdiff, d: sdiff,  g: ndiff });
        }
    };
    ...
    const changeXaxis = (x) => {
        if (x !== gridNums.c) {
            ...
            let compSum = (slideLock.b ? 0 : gridNums.b) + (slideLock.a ? 0 : gridNums.a) + (slideLock.d ? 0 : gridNums.d) + (slideLock.g ? 0 : gridNums.g);
            //console.log(diff)
            let gdiff = (slideLock.b ? gridNums.b : gridNums.b - (gridNums.b / compSum) * diff);
            let tdiff = (slideLock.a ? gridNums.a : gridNums.a - (gridNums.a / compSum) * diff);
            let sdiff = (slideLock.d ? gridNums.d : gridNums.d - (gridNums.d / compSum) * diff);
            let ndiff = (slideLock.g ? gridNums.g : gridNums.g - (gridNums.g / compSum) * diff);
            //console.log(x,gdiff,xdiff,sdiff,(x+gdiff+xdiff+sdiff))
            setGridNums({ ...gridNums, a: tdiff, b: gdiff, c: x, d: sdiff, g: ndiff });
        }
    };
    ...
    const changeSource = (x) => {
        if (x !== gridNums.d) {
            ...
            let compSum = (slideLock.b ? 0 : gridNums.b) + ( slideLock.c ? 0 : gridNums.c ) + (slideLock.a ? 0 : gridNums.a) + (slideLock.g ? 0 : gridNums.g);
            //console.log(diff)
            let gdiff = (slideLock.b ? gridNums.b : gridNums.b - (gridNums.b / compSum) * diff);
            let xdiff = (slideLock.c ? gridNums.c : gridNums.c - (gridNums.c / compSum) * diff);
            let tdiff = (slideLock.a ? gridNums.a : gridNums.a - (gridNums.a / compSum) * diff);
            let ndiff = (slideLock.g ? gridNums.g : gridNums.g - (gridNums.g / compSum) * diff);
            //console.log(x,gdiff,xdiff,sdiff,(x+gdiff+xdiff+sdiff))
            setGridNums({ ...gridNums, a: tdiff, b: gdiff, c: xdiff, d: x, g: ndiff });
        }
    };

    const changeNewRow = (x) => {
        if (x !== gridNums.g) {
            let diff = x - gridNums.g;
            let compSum = (slideLock.b ? 0 : gridNums.b) + ( slideLock.c ? 0 : gridNums.c ) + (slideLock.a ? 0 : gridNums.a) + (slideLock.d ? 0 : gridNums.d);
            //console.log(diff)
            let gdiff = (slideLock.b ? gridNums.b : gridNums.b - (gridNums.b / compSum) * diff);
            let xdiff = (slideLock.c ? gridNums.c : gridNums.c - (gridNums.c / compSum) * diff);
            let tdiff = (slideLock.a ? gridNums.a : gridNums.a - (gridNums.a / compSum) * diff);
            let sdiff = (slideLock.d ? gridNums.d : gridNums.d - (gridNums.d / compSum) * diff);
            //console.log(x,gdiff,xdiff,sdiff,(x+gdiff+xdiff+sdiff))
            setGridNums({ ...gridNums, a: tdiff, b: gdiff, c: xdiff, d: sdiff, g: x });
        }
    };
    ...
    return (
        <div>
            ...
            <div className="slide-obj">
                <div className="slide-row">
                    <span className="txt-group">
                        <p className="slider-tag">Top X-Axis Height</p>
                        <div className="input-group">
                            <input type="number" className="input-num" value={gridNums.g} onChange={(e) => {if(isNaN(parseInt(e.target.value))){changeNewRow(gridNums.d)}else if(e.target.value<=100){changeNewRow(parseInt(e.target.value))}}} min={0} max={100} disabled={slideLock.g}/>
                            <p className='percent'>%</p>
                        </div>
                    </span>
                    <button className={`grid-toggle ${slideLock.g ? '' : 'toggle-down'}`} onClick={() => setSlideLock({...slideLock, g: !slideLock.g})}>
                        { slideLock.g ?
                        <i className="fas fa-lock"></i>
                        :
                        <i className="fas fa-unlock"></i>  
                ...
                xmin={0}
                xmax={100-lockedHeight}
                disabled={slideLock.g}
                x={gridNums.g}
                onChange={({ x }) => changeNewRow(x)}
                styles={{track: {background: 'none', boxShadow: 'inset .15rem .15rem 6px #586c8a7e, inset -.25rem -.25rem 9px #ffffff'}, active: { background: 'linear-gradient(90deg, #C7E8E8 0%, #83DBD6 86.69%)', boxShadow: '-4px 4px 12px #83DBD6'}}}
                />
            </div>
        </div>
